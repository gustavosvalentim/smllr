...
class FingerprintTestCase(TestCase):
    ...
    def test_fingerprint_creation(self):
        ...
        assert fingerprint.get("ip_address") == expects_ip_address, (
            f"IP address should be '{expects_ip_address}'"
        )
        assert fingerprint.get("browser_name") == expects_browser_name, (
            f"Browser name should be '{expects_browser_name}'"
        )
        assert fingerprint.get("browser_version") == expects_browser_version, (
            f"Browser version should be '{expects_browser_version}'"
        )
        assert fingerprint.get("os") == expects_os_name, (
