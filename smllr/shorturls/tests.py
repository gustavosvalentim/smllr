...
class ShortURLTestCase(TestCase):
    ...
    def test_anon_user_shorturl_limit(self):
        for i in range(settings.MAX_SHORTURLS_PER_ANON_USER):
            ShortURL.objects.create(user=self.user, destination="https://smllr.io", name=f"Test {i}")

        with self.assertRaises(Exception) as ex:
            ShortURL.objects.create(user=self.user, destination="https://smllr.io", name="Error")
